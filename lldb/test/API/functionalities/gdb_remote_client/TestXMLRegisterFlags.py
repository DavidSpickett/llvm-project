...
class TestXMLRegisterFlags(GDBRemoteTestBase):
    ...
    def test_enum_duplicated(self):
        ...
        self.setup_register_test(
            """\
          <enum id="some_enum" size="4">
            <evalue name="abc" value="0"/>
          </enum>
          <enum id="some_enum" size="4">
            <evalue name="def" value="0"/>
          </enum>
          <flags id="cpsr_flags" size="4">
            <field name="E" start="0" end="0" type="some_enum"/>
          </flags>
        ...
        self.expect("register info cpsr", patterns=["E: 0 = def$"])
    ...
    def test_evalue_empty_name(self):
        ...
        # declared later so that its name is hidden should the debugger only
    ...
    def test_enum_unknown_value(self):
        """Check that lldb prints the value of the field even if it does not
        match one of the evalues."""

        self.setup_register_test(
            """\
          <enum id="foo_enum" size="8">
            <evalue name="unknown" value="0"/>
          </enum>
          <enum id="bar_enum" size="8">
            <evalue name="known" value="1"/>
          </enum>
          <flags id="x0_flags" size="8">
            <field name="foo" start="0" end="0" type="foo_enum"/>
            <field name="bar" start="1" end="1" type="bar_enum"/>
          </flags>
          <reg name="pc" bitsize="64"/>
          <reg name="x0" regnum="0" bitsize="64" type="x0_flags"/>
          <reg name="cpsr" regnum="33" bitsize="32"/>"""
        ...
        self.expect("register read x0", substrs=["(bar = known (0x1), foo = 0x1)"])

    @skipIfXmlSupportMissing
    @skipIfRemote
    def test_bitfield_like_enum(self):
        """Check that register read can handle enums that look like bitfields,
        where each evalue is a single bit."""

        self.setup_register_test(
            """\
          <enum id="foo_enum" size="8">
            <evalue name="bit_0" value="1"/>
            <evalue name="bit_1" value="2"/>
            <evalue name="bit_2" value="4"/>
            <evalue name="bit_3" value="8"/>
          </enum>
          <flags id="x0_flags" size="8">
            <field name="lower" start="0" end="3" type="foo_enum"/>
            <field name="upper" start="32" end="35" type="foo_enum"/>
          </flags>
          <reg name="pc" bitsize="64"/>
          <reg name="x0" regnum="0" bitsize="64" type="x0_flags"/>
          <reg name="cpsr" regnum="33" bitsize="32"/>"""
        ...
        self.expect(
            "register read x0",
            substrs=[
                "(upper = bit_1 (0x2) | bit_2 (0x4) | bit_3 (0x8), "
                "lower = bit_0 (0x1) | bit_1 (0x2) | bit_2 (0x4))"
            ],
        )
