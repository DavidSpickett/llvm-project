...
These map to the regiters ptrace provides. The first two have had a ``gcs_``
...
``gcspr_el0`` then points to (as opposed to using the GCS push instructions).
...
choice was to not restore the enable bit at all. It's up to the user and/or
