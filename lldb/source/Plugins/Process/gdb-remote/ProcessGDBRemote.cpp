...
static FieldEnum::Enumerators ParseEnumEvalues(const XMLNode &enum_node) {
  Log *log(GetLog(GDBRLog::Process));
  // We will use the last instance of each value. This is map specifically so
  // that we preserve the order seen in the XML, which may not be numerical.
  ...
  enum_node.ForEachChildElementWithName(
      "evalue", [&enumerators, &log](const XMLNode &enumerator_node) {
            ...
            [&name, &value, &log](const llvm::StringRef &attr_name,
              ...
              if (attr_name == "name") {
                ...
                if (llvm::to_integer(attr_value, parsed_value)) {
                  value = parsed_value;
                  // We do not add to the seen values here, because the
                  // enumerator may still fail to parse.
                } else
                  LLDB_LOG(log,
        ...
        // Keep going to find all evalue elements.
        return true;
      });
  ...
  feature_node.ForEachChildElementWithName(
      "enum", [log, &registers_enum_types](const XMLNode &enum_node) {
        ...
        enum_node.ForEachAttribute([&id](const llvm::StringRef &attr_name,
          ...
          // * It's very annoying to verify because you have to wait until the
          //   enum is applied to a field.
          //
          // So we will emit this attribute in XML, for GDB's sake, but won't
          // bother ingesting it.
